    ...
    if quote and quote['c'] != 0:
        ...
        if df_live is not None:
            ...
            if show_whales:
                ...
                fig.add_trace(go.Scatter(x=df_live.index, y=df_live['Whale'], mode='markers', name="Whale Spike", marker=dict(size=10, color="orange", symbol="diamond")))
            fig.update_layout(template="plotly_dark", height=450)
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.warning("⚠️ Alpha Vantage Limit Reached. Charting will resume tomorrow.")
